...
router.get('/health-summary/pdf', authMiddleware, async (req, res) => {
  try {
    const [user, metrics, medicines, symptoms] = await Promise.all([
      ...
      SymptomCheck.find({ user: req.user.id }).sort({ checkedAt: -1 }).limit(100),
    ]);
    ...
    const pdfBuffer = await generateHealthReportPDF(user, metrics, medicines, symptoms);
